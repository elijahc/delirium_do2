...
class Table(object):
    ...
    def nunique(self, column=None):
        column = column or self.default_col
        assert column is not None
            
        return csv.read_csv(self.file_path,convert_options=csv.ConvertOptions(include_columns=[column]))[column].to_pandas().astype(str).str.upper().value_counts()
    ...
    def unique(self, column=None, count=True):
        ...
        if self.default_unique is None:
            self.default_unique = csv.read_csv(self.file_path,convert_options=csv.ConvertOptions(include_columns=[column]))[column].to_pandas().astype(str).str.upper().unique()
